...
Bit vector size \textbf{n}, rank and select in time \bigO{1}, stucture size $n + o(n)$ bits\\
\textbf{Rank:} number of 1s in $A[0 \dots i]$. Structure is 2lvl hierarchy, superblocks (size $t_1=\log^2 n$, keep global rank at boundary) within them are blocks (size $t_2=\frac{1}{2}\log n$, keep rank (within superblock) at boundary). Block size $t_2$, all possible blocks $2^{t_2}$, total memory \bigO{2^{t_2} t_2 \log{t_2}} = \bigO{\sqrt{n} \log{n} \log \log n} = $o(n)$ bits\\
