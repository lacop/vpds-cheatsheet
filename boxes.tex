...
\defbox{\boxTEXTmpp}{50}{32}{MP sp table}{
\scriptsize
\begin{algorithmic}
\State $sp[0]=sp[1]=0$
\State $j=0$
...
\defbox{\boxBW}{90}{65}{BW}{
\textbf{const:} sort all rotations of string lexicographically. out: last column. \\
\textbf{reverse:} sort lex. to obtain First column. $F[i]$ follows $L[i]$. jth occurrence of x in F is the same as jth occurrence of x in L. LF[i] = row j in which F[j] corresponds to L[i] \\
...
\textbf{counting occ of P (FM index)} search for string backwards using LF transformation \\
